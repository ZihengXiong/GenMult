...
func (m *Manager) sendWithConfig(ctx context.Context, sender Sender, cfg ChannelConfig, msg OutboundMessage, policy OutboundPolicy) error {
	...
	for i := 0; i < policy.RetryMax; i++ {
		...
		if err == nil {
			...
			m.dispatchOutboundBotMentions(ctx, cfg, target, normalized.Message)
			return nil
		}
		lastErr = err
		if m.logger != nil {
...
func (s *managerOutboundStream) Push(ctx context.Context, event StreamEvent) error {
	...
	if event.Type == StreamEventFinal && event.Final != nil && s.send != nil {
		var err error
		if s.splitCount > 0 {
			err = s.pushFinalAfterSplit(ctx, event)
		} else {
			err = s.pushFinalWithChunking(ctx, event)
		}
		if err == nil {
			s.manager.dispatchOutboundBotMentions(ctx, s.config, s.target, event.Final.Message)
		}
		return err
	}
