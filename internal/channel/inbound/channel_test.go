...
func (f *fakeChatService) ListLatestBySession(_ context.Context, sessionID string, limit int32) ([]messagepkg.Message, error) {
	f.latestSession = sessionID
	f.latestLimit = limit
	return f.latest, nil
}

func testPersistedModelMessage(role, text, senderID, externalID, replyToID, routeID string, createdAt time.Time) messagepkg.Message {
	content, _ := json.Marshal(conversation.ModelMessage{
		Role:    role,
	...
	return messagepkg.Message{
		...
		SourceReplyToMessageID:  replyToID,
...
func TestChannelInboundProcessorGroupAmbientDirectAddressTriggersReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-ambient"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{ChatID: "chat-ambient", RouteID: "route-ambient"}}
	gateway := &fakeChatGateway{
		resp: conversation.ChatResponse{
			Messages: []conversation.ModelMessage{
				{Role: "assistant", Content: conversation.NewTextContent("AI reply")},
	...
	msg := channel.InboundMessage{
		BotID:       "bot-1",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-ambient-1", Text: "你有什么 skills？"},
		ReplyTarget: "chat_id:oc_ambient",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_ambient",
			Type: "group",
	...
	if gateway.gotReq.Query == "" {
		t.Fatalf("group direct-address message should trigger chat call")
	}
	if len(sender.sent) != 1 {
		t.Fatalf("expected one outbound reply, got %d", len(sender.sent))
	}
...
func TestChannelInboundProcessorGroupEntryModeWeatherTriggersReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-entry-weather"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{ChatID: "chat-entry-weather", RouteID: "route-entry-weather"}}
	callCount := 0
	gateway := &fakeChatGateway{
		resp: conversation.ChatResponse{
			Messages: []conversation.ModelMessage{
				{Role: "assistant", Content: conversation.NewTextContent("今天香港有雨。")},
			},
		},
		onChat: func(conversation.ChatRequest) {
			callCount++
	...
	cfg := channel.ChannelConfig{
		...
		Routing: map[string]any{
			"group_trigger_mode": "entry",
			"worker_names":       []any{"乔小治"},
		},
	}
	msg := channel.InboundMessage{
		BotID:       "bot-entry",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-entry-weather", Text: "帮我查一下今天香港天气"},
		ReplyTarget: "chat_id:oc_entry_weather",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_entry_weather",
			Type: "group",
	...
	if callCount != 1 {
		t.Fatalf("entry mode weather request should trigger chat call, got %d calls", callCount)
	}
	if len(sender.sent) != 1 {
		t.Fatalf("expected one outbound reply, got %d", len(sender.sent))
	}
...
func TestChannelInboundProcessorGroupDirectedOnlySuppressesAmbient(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-worker-passive"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{ChatID: "chat-worker-passive", RouteID: "route-worker-passive"}}
	callCount := 0
	gateway := &fakeChatGateway{
		resp: conversation.ChatResponse{
			Messages: []conversation.ModelMessage{
	...
	cfg := channel.ChannelConfig{
		ID:           "cfg-worker",
		BotID:        "bot-worker",
		SelfIdentity: map[string]any{"name": "乔小治"},
		Routing:      map[string]any{"group_trigger_mode": "directed_only"},
	}
	msg := channel.InboundMessage{
		BotID:       "bot-worker",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-worker-passive", Text: "帮我查一下今天香港天气"},
		ReplyTarget: "chat_id:oc_worker_passive",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_worker_passive",
			Type: "group",
		},
	}
	...
	if callCount != 0 {
		t.Fatalf("directed_only worker should not answer ambient group request, got %d calls", callCount)
	}
	if len(chatSvc.persisted) != 1 {
		t.Fatalf("passive worker message should be persisted once, got %d", len(chatSvc.persisted))
	}
}

func TestChannelInboundProcessorGroupDirectedOnlyNamedExecutionTriggersReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-worker-exec"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{ChatID: "chat-worker-exec", RouteID: "route-worker-exec"}}
	callCount := 0
	gateway := &fakeChatGateway{
		resp: conversation.ChatResponse{
			Messages: []conversation.ModelMessage{
				{Role: "assistant", Content: conversation.NewTextContent("已执行。")},
			},
		},
		onChat: func(conversation.ChatRequest) {
			callCount++
	...
	msg := channel.InboundMessage{
		BotID:       "bot-worker",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-worker-exec", Text: "乔小治执行任务：检查登录模块"},
		ReplyTarget: "chat_id:oc_worker_exec",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_worker_exec",
			Type: "group",
	...
	if callCount != 1 {
		t.Fatalf("named worker execution should trigger chat call, got %d calls", callCount)
	}
	if len(sender.sent) != 1 {
		t.Fatalf("expected one outbound reply, got %d", len(sender.sent))
	}
...
func TestChannelInboundProcessorGroupAmbientContinuationAfterMentionTriggersReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-continuation"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{ChatID: "chat-continuation", RouteID: "route-continuation"}}
	callCount := 0
	gateway := &fakeChatGateway{
		resp: conversation.ChatResponse{
			Messages: []conversation.ModelMessage{
	...
	cfg := channel.ChannelConfig{ID: "cfg-1", BotID: "bot-1"}
	first := channel.InboundMessage{
		BotID:       "bot-1",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-continuation-1", Text: "@bot 这个怎么配置"},
		ReplyTarget: "chat_id:oc_continuation",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_continuation",
			Type: "group",
	...
	if err := processor.HandleInbound(context.Background(), cfg, first, sender); err != nil {
		t.Fatalf("first HandleInbound: %v", err)
	}

	second := channel.InboundMessage{
		BotID:       "bot-1",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-continuation-2", Text: "那为什么会这样呢"},
		ReplyTarget: "chat_id:oc_continuation",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_continuation",
	...
	if err := processor.HandleInbound(context.Background(), cfg, second, sender); err != nil {
		t.Fatalf("second HandleInbound: %v", err)
	}
	if callCount != 2 {
		t.Fatalf("expected continuation to trigger second chat call, got %d calls", callCount)
	}
	if got := strings.TrimSpace(gateway.gotReq.Query); got != "那为什么会这样呢" {
		t.Fatalf("unexpected continuation query: %q", got)
	}
}

func TestChannelInboundProcessorGroupAmbientHistoryContinuationTriggersReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-history"}}
	policySvc := &fakePolicyService{}
	now := time.Now()
	chatSvc := &fakeChatService{
		resolveResult: route.ResolveConversationResult{ChatID: "chat-history", RouteID: "route-history"},
		latest: []messagepkg.Message{
			testPersistedModelMessage("assistant", "可以这样配置。", "", "assistant-history-1", "user-history-1", "route-history", now.Add(-10*time.Second)),
			testPersistedModelMessage("user", "这个怎么配置", "channelIdentity-history", "user-history-1", "", "route-history", now.Add(-20*time.Second)),
		},
	}
	callCount := 0
	gateway := &fakeChatGateway{
	...
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, gateway, channelIdentitySvc, policySvc, "", 0)
	processor.SetSessionEnsurer(&fakeSessionEnsurer{activeSession: SessionResult{ID: "session-history", Type: "chat"}})
	sender := &fakeReplySender{}
	...
	msg := channel.InboundMessage{
		BotID:       "bot-1",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-history-2", Text: "那具体怎么做呢"},
		ReplyTarget: "chat_id:oc_history",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_history",
			Type: "group",
	...
	if callCount != 1 {
		t.Fatalf("expected persisted history continuation to trigger chat call, got %d calls", callCount)
	}
	if chatSvc.latestSession != "session-history" {
		t.Fatalf("expected latest history lookup by active session, got %q", chatSvc.latestSession)
	}
}

func TestChannelInboundProcessorGroupAmbientHistoryOtherUserAfterAssistantStaysPassive(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-history-passive"}}
	policySvc := &fakePolicyService{}
	now := time.Now()
	chatSvc := &fakeChatService{
		resolveResult: route.ResolveConversationResult{ChatID: "chat-history-passive", RouteID: "route-history-passive"},
		latest: []messagepkg.Message{
			testPersistedModelMessage("user", "我插一句", "other-channel-identity", "other-history-1", "", "route-history-passive", now.Add(-5*time.Second)),
			testPersistedModelMessage("assistant", "可以这样配置。", "", "assistant-history-1", "user-history-1", "route-history-passive", now.Add(-10*time.Second)),
			testPersistedModelMessage("user", "这个怎么配置", "channelIdentity-history-passive", "user-history-1", "", "route-history-passive", now.Add(-20*time.Second)),
		},
	}
	callCount := 0
	gateway := &fakeChatGateway{
	...
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, gateway, channelIdentitySvc, policySvc, "", 0)
	processor.SetSessionEnsurer(&fakeSessionEnsurer{activeSession: SessionResult{ID: "session-history-passive", Type: "chat"}})
	sender := &fakeReplySender{}
	...
	msg := channel.InboundMessage{
		BotID:       "bot-1",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-history-passive-2", Text: "那具体怎么做呢"},
		ReplyTarget: "chat_id:oc_history_passive",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_history_passive",
			Type: "group",
		},
	}
	if err := processor.HandleInbound(context.Background(), cfg, msg, sender); err != nil {
	...
	if callCount != 0 {
		t.Fatalf("other user's newer group message should keep current message passive, got %d calls", callCount)
	}
	if len(chatSvc.persisted) != 1 {
		t.Fatalf("passive message should persist once, got %d", len(chatSvc.persisted))
	}
}

func TestChannelInboundProcessorGroupAmbientReplyToAssistantHistoryTriggersReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-reply-history"}}
	policySvc := &fakePolicyService{}
	now := time.Now()
	chatSvc := &fakeChatService{
		resolveResult: route.ResolveConversationResult{ChatID: "chat-reply-history", RouteID: "route-reply-history"},
		latest: []messagepkg.Message{
			testPersistedModelMessage("assistant", "上一条回复", "", "assistant-reply-history-1", "user-reply-history-1", "route-reply-history", now.Add(-10*time.Second)),
		},
	}
	callCount := 0
	gateway := &fakeChatGateway{
	...
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, gateway, channelIdentitySvc, policySvc, "", 0)
	processor.SetSessionEnsurer(&fakeSessionEnsurer{activeSession: SessionResult{ID: "session-reply-history", Type: "chat"}})
	sender := &fakeReplySender{}
	...
	msg := channel.InboundMessage{
		...
		Channel: channel.ChannelType("feishu"),
		Message: channel.Message{
			ID:    "msg-reply-history-2",
			Text:  "这个不太对",
			Reply: &channel.ReplyRef{MessageID: "assistant-reply-history-1"},
		},
		ReplyTarget: "chat_id:oc_reply_history",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_reply_history",
			Type: "group",
	...
	if callCount != 1 {
		t.Fatalf("replying to recent assistant message should trigger chat call, got %d calls", callCount)
	}
}

func TestChannelInboundProcessorGroupAmbientOtherTargetStaysPassive(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identities.ChannelIdentity{ID: "channelIdentity-other-target"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{ChatID: "chat-other-target", RouteID: "route-other-target"}}
	callCount := 0
	gateway := &fakeChatGateway{
		resp: conversation.ChatResponse{
			Messages: []conversation.ModelMessage{
	...
	first := channel.InboundMessage{
		BotID:       "bot-1",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-other-target-1", Text: "@bot 先说一下"},
		ReplyTarget: "chat_id:oc_other_target",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_other_target",
			Type: "group",
	...
	second := channel.InboundMessage{
		BotID:       "bot-1",
		Channel:     channel.ChannelType("feishu"),
		Message:     channel.Message{ID: "msg-other-target-2", Text: "@alex 那为什么会这样呢"},
		ReplyTarget: "chat_id:oc_other_target",
		Sender:      channel.Identity{SubjectID: "user-1"},
		Conversation: channel.Conversation{
			ID:   "oc_other_target",
	...
	if callCount != 1 {
		t.Fatalf("other-target message should stay passive, got %d calls", callCount)
	}
	if len(chatSvc.persisted) != 1 {
		t.Fatalf("other-target passive message should persist once, got %d", len(chatSvc.persisted))
	}
}
