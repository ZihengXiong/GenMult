...
const (
	...
	groupAmbientReplyWindow = 5 * time.Minute
	groupAmbientHistorySize = 12
)
...
type groupAmbientHistoryReader interface {
...
func (p *ChannelInboundProcessor) HandleInbound(ctx context.Context, cfg channel.ChannelConfig, msg channel.InboundMessage, sender channel.StreamReplySender) (retErr error) {
	...
	baseTrigger := shouldTriggerAssistantResponse(msg) || identity.ForceReply
	ambientTrigger := false
	if !baseTrigger {
		ambientTrigger = p.shouldTriggerGroupAmbientReply(ctx, cfg, identity, msg, strings.TrimSpace(resolved.RouteID), sessionID, text)
	}
	shouldTrigger := baseTrigger || ambientTrigger
	if shouldTrigger {
		p.rememberGroupReplyContext(identity, msg, strings.TrimSpace(resolved.RouteID))
	}
...
type groupReplyContextState struct {
	SenderID string
	At       time.Time
}

type groupAmbientMessage struct {
	Role                   string
	SenderID               string
	Text                   string
...
func (p *ChannelInboundProcessor) shouldTriggerGroupAmbientReply(ctx context.Context, cfg channel.ChannelConfig, identity InboundIdentity, msg channel.InboundMessage, routeID, sessionID, text string) bool {
	if p == nil || !isGroupConversationType(msg.Conversation.Type) {
		return false
	}
	if strings.TrimSpace(routeID) == "" || strings.TrimSpace(text) == "" {
		return false
	}
	switch groupTriggerMode(cfg) {
	case groupTriggerModeDirectedOnly:
		return isNamedExecutionRequest(text, cfg)
	case groupTriggerModeEntry:
		if addressesConfiguredPeerBot(text, cfg) {
			return false
		}
		if looksLikeEntryAssistantRequest(text) {
			return true
		}
	}
	if messageMentionsAnotherTarget(text) {
		return false
	}
	recent := p.loadRecentGroupAmbientMessages(ctx, sessionID, routeID)
	if ambientReplyTargetsRecentAssistant(msg, recent) {
		return true
	}
	if directlyAddressesAssistant(text) {
		return true
	}
	if !looksLikeFollowUpToAssistant(text) {
		return false
	}
	if p.hasRecentGroupReplyContext(identity, routeID) {
		return true
	}
	return recentGroupHistoryShowsAssistantContinuation(identity, recent)
}

const (
	groupTriggerModeEntry        = "entry"
	groupTriggerModeDirectedOnly = "directed_only"
)

func groupTriggerMode(cfg channel.ChannelConfig) string {
	return strings.ToLower(strings.TrimSpace(stringFromMapAny(cfg.Routing, "group_trigger_mode", "groupTriggerMode", "group_trigger", "groupTrigger")))
}

func isNamedExecutionRequest(text string, cfg channel.ChannelConfig) bool {
	normalized := normalizeAmbientText(text)
	if normalized == "" {
		return false
	}
	names := configuredBotNames(cfg)
	names = append(names, stringsFromMapAny(cfg.Routing, "execute_names", "executeNames")...)
	for _, name := range names {
		name = strings.ToLower(strings.TrimSpace(name))
		if name == "" {
			continue
		}
		if strings.HasPrefix(normalized, name+"执行") ||
			strings.HasPrefix(normalized, name+"，执行") ||
			strings.HasPrefix(normalized, name+",执行") ||
			strings.HasPrefix(normalized, name+" 帮") ||
			strings.HasPrefix(normalized, name+"帮") {
			return true
		}
	}
	return strings.HasPrefix(normalized, "执行任务：") || strings.HasPrefix(normalized, "执行任务:")
}

func addressesConfiguredPeerBot(text string, cfg channel.ChannelConfig) bool {
	normalized := normalizeAmbientText(text)
	if normalized == "" {
		return false
	}
	for _, name := range stringsFromMapAny(cfg.Routing, "peer_bot_names", "peerBotNames", "worker_names", "workerNames") {
		name = strings.ToLower(strings.TrimSpace(name))
...
func looksLikeEntryAssistantRequest(text string) bool {
	normalized := normalizeAmbientText(text)
	if normalized == "" {
		return false
	}
	if containsAny(normalized, "天气", "气温", "下雨", "降雨", "温度", "空气质量", "台风", "预报") {
		return true
	}
	return strings.HasPrefix(normalized, "帮我") ||
		strings.HasPrefix(normalized, "帮忙") ||
		strings.HasPrefix(normalized, "请") ||
		strings.HasPrefix(normalized, "麻烦") ||
		strings.HasPrefix(normalized, "查一下") ||
		strings.HasPrefix(normalized, "搜一下") ||
		strings.HasPrefix(normalized, "看一下") ||
		strings.HasPrefix(normalized, "总结") ||
		strings.HasPrefix(normalized, "整理")
}

func configuredBotNames(cfg channel.ChannelConfig) []string {
	names := make([]string, 0, 2)
	if name := strings.TrimSpace(stringFromMapAny(cfg.SelfIdentity, "name", "display_name", "displayName")); name != "" {
		names = append(names, name)
	}
	names = append(names, stringsFromMapAny(cfg.Routing, "bot_names", "botNames")...)
	return names
}
...
func stringsFromMapAny(m map[string]any, keys ...string) []string {
	for _, key := range keys {
		if m == nil {
			return nil
		}
...
func (p *ChannelInboundProcessor) hasRecentGroupReplyContext(identity InboundIdentity, routeID string) bool {
	if p == nil {
		return false
	}
	raw, ok := p.groupReplyContext.Load(groupReplyContextKey(identity.BotID, routeID))
	if !ok {
		return false
	}
	state, ok := raw.(groupReplyContextState)
	if !ok || strings.TrimSpace(state.SenderID) == "" {
		return false
	}
	if !sameGroupParticipant(state.SenderID, identity) {
		return false
	}
	if state.At.IsZero() || time.Since(state.At) > groupAmbientReplyWindow {
		return false
	}
	return true
}

func (p *ChannelInboundProcessor) loadRecentGroupAmbientMessages(ctx context.Context, sessionID, routeID string) []groupAmbientMessage {
	if p == nil || strings.TrimSpace(sessionID) == "" {
		return nil
	}
	reader, ok := p.message.(groupAmbientHistoryReader)
	if !ok || reader == nil {
		return nil
	}
	messages, err := reader.ListLatestBySession(ctx, strings.TrimSpace(sessionID), groupAmbientHistorySize)
	if err != nil {
		if p.logger != nil {
			p.logger.Warn("load recent group ambient history failed", slog.Any("error", err), slog.String("session_id", strings.TrimSpace(sessionID)))
		}
		return nil
	}
	recent := make([]groupAmbientMessage, 0, len(messages))
	for _, m := range messages {
		if route := strings.TrimSpace(messageRouteID(m)); route != "" && strings.TrimSpace(routeID) != "" && route != strings.TrimSpace(routeID) {
			continue
		}
		recent = append(recent, groupAmbientMessage{
			Role:                   strings.TrimSpace(m.Role),
			SenderID:               firstNonEmpty(m.SenderChannelIdentityID, m.SenderUserID),
			Text:                   persistedMessageText(m),
			ExternalMessageID:      strings.TrimSpace(m.ExternalMessageID),
			SourceReplyToMessageID: strings.TrimSpace(m.SourceReplyToMessageID),
...
func (p *ChannelInboundProcessor) rememberGroupReplyContext(identity InboundIdentity, msg channel.InboundMessage, routeID string) {
	if p == nil || !isGroupConversationType(msg.Conversation.Type) {
		return
	}
	key := groupReplyContextKey(identity.BotID, routeID)
	if key == "" {
		return
	}
	senderID := groupParticipantID(identity)
	if senderID == "" {
		return
	}
	p.groupReplyContext.Store(key, groupReplyContextState{
		SenderID: senderID,
		At:       time.Now(),
	})
}

func groupReplyContextKey(botID, routeID string) string {
	botID = strings.TrimSpace(botID)
	routeID = strings.TrimSpace(routeID)
	if botID == "" || routeID == "" {
		return ""
	}
	return botID + ":" + routeID
}

func groupParticipantID(identity InboundIdentity) string {
	for _, candidate := range []string{identity.ChannelIdentityID, identity.SubjectID, identity.UserID} {
...
func sameGroupParticipant(senderID string, identity InboundIdentity) bool {
	senderID = strings.TrimSpace(senderID)
	if senderID == "" {
		return false
	}
	return senderID == strings.TrimSpace(identity.ChannelIdentityID) ||
		senderID == strings.TrimSpace(identity.SubjectID) ||
		senderID == strings.TrimSpace(identity.UserID)
}

func recentGroupHistoryShowsAssistantContinuation(identity InboundIdentity, recent []groupAmbientMessage) bool {
	if len(recent) == 0 || groupParticipantID(identity) == "" {
		return false
	}
	assistantIdx := -1
	for i, item := range recent {
		if !messageInsideAmbientWindow(item.CreatedAt) {
			continue
		}
		if strings.EqualFold(strings.TrimSpace(item.Role), "assistant") && strings.TrimSpace(item.Text) != "" {
			assistantIdx = i
			break
		}
	}
	...
	for _, item := range recent[:assistantIdx] {
		if !strings.EqualFold(strings.TrimSpace(item.Role), "user") {
			continue
		}
		if !sameGroupParticipant(item.SenderID, identity) {
			return false
		}
	}
	for _, item := range recent[assistantIdx+1:] {
		if !messageInsideAmbientWindow(item.CreatedAt) {
			continue
		}
		if strings.EqualFold(strings.TrimSpace(item.Role), "user") && sameGroupParticipant(item.SenderID, identity) {
			return true
		}
	}
	return false
...
func ambientReplyTargetsRecentAssistant(msg channel.InboundMessage, recent []groupAmbientMessage) bool {
	replyID := strings.TrimSpace(inboundReplyMessageID(msg.Message.Reply))
	if replyID == "" || len(recent) == 0 {
		return false
	}
	for _, item := range recent {
		...
		if strings.TrimSpace(item.ExternalMessageID) == replyID {
			return true
		}
	}
	return false
...
func messageInsideAmbientWindow(t time.Time) bool {
	return t.IsZero() || time.Since(t) <= groupAmbientReplyWindow
}

func messageRouteID(msg messagepkg.Message) string {
	if msg.Metadata == nil {
		return ""
	}
	if v, ok := msg.Metadata["route_id"].(string); ok {
		return strings.TrimSpace(v)
	}
	return ""
}

func persistedMessageText(msg messagepkg.Message) string {
	if text := strings.TrimSpace(msg.DisplayContent); text != "" {
		return text
	}
	var model conversation.ModelMessage
	if err := json.Unmarshal(msg.Content, &model); err == nil {
		if text := strings.TrimSpace(model.TextContent()); text != "" {
			return stripMessageEnvelope(text)
		}
	}
	var text string
...
func stripMessageEnvelope(text string) string {
	text = strings.TrimSpace(text)
	if text == "" || !strings.HasPrefix(text, "<message") {
		return text
	}
	start := strings.Index(text, ">")
	end := strings.LastIndex(text, "</message>")
...
func isGroupConversationType(conversationType string) bool {
	return channel.NormalizeConversationType(conversationType) == channel.ConversationTypeGroup
}

func directlyAddressesAssistant(text string) bool {
	normalized := normalizeAmbientText(text)
	hasSecondPerson := strings.Contains(normalized, "你")
	if !hasSecondPerson && !containsAny(normalized, "bot", "chatbot", "机器人", "ai", "agent", "助手") {
		return false
	}
	if containsAny(normalized,
		"skills", "skill", "工具", "记忆", "模型", "人设", "prompt", "提示词",
		"回复", "回答", "为什么", "怎么", "如何", "什么", "名字", "是谁",
		"区别", "不同", "两个", "你俩", "你们",
	) {
		return true
	}
	if hasQuestionMark(normalized) && containsAny(normalized, "能", "会", "可以", "要不要", "是不是", "有没有") {
		return true
	}
	return false
}

func looksLikeFollowUpToAssistant(text string) bool {
	normalized := normalizeAmbientText(text)
	if hasQuestionMark(normalized) && containsAny(normalized, "为什么", "怎么", "如何", "什么", "吗", "呢", "可以", "能不能") {
		return true
	}
	return containsAny(normalized,
		"那", "这个", "这条", "上面", "刚才", "继续", "展开", "具体",
		"为什么", "怎么", "如何", "什么意思", "然后呢", "所以呢", "还有呢",
		"可以吗", "对吗", "是不是", "能不能",
	)
}

func messageMentionsAnotherTarget(text string) bool {
	normalized := normalizeAmbientText(text)
	if strings.Contains(normalized, "@") || strings.Contains(normalized, "＠") {
		return true
	}
	return containsAny(normalized, "问一下", "帮他", "帮她", "跟他说", "跟她说", "你们聊", "他刚才", "她刚才")
}

func normalizeAmbientText(text string) string {
