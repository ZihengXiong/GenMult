    ...
    >
      ...
      >
        <div class="h-10 flex items-center pl-2 group-data-[collapsible=icon]:pl-3 transition-[padding] duration-200 ease-linear">
          ...
          <div
            v-if="showBotList"
            class="ml-auto mr-1.5 group-data-[collapsible=icon]:hidden"
          >
            <Button
              variant="ghost"
              size="icon"
        ...
        <SidebarMenu class="gap-2.5">
          <SidebarMenuItem>
            <SidebarMenuButton
              ...
              :is-active="isAgentHubActive"
              @click="router.push({ name: 'agent-hub' })"
            >
              <Network
                class="size-3.5"
              />
              <span class="text-xs font-medium group-data-[collapsible=icon]:hidden">AgentHub</span>
            </SidebarMenuButton>
          </SidebarMenuItem>
          <SidebarMenuItem>
            <SidebarMenuButton
...
import { Plus, LoaderCircle, Settings, PanelLeftClose, PanelLeftOpen, Network } from 'lucide-vue-next'
...
const bots = computed<BotsBot[]>(() => sortBots(visibleBots(botData.value?.items ?? [])))

const isSettingsActive = computed(() => route.path.startsWith('/settings'))
const isAgentHubActive = computed(() => route.name === 'home' || route.name === 'agent-hub')
const showBotList = computed(() => route.name === 'chat')
</script>
