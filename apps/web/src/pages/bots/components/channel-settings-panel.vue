<template>
  <div class="space-y-6 mr-4">
    ...
    <div v-else-if="channelItem.meta.type === 'whatsapp'">
      <WhatsAppQrLogin
        :bot-id="botId"
        :proxy="whatsappProxy"
        :client-name="whatsappClientName"
...
const whatsappProxy = computed(() =>
  String(form.credentials.proxy ?? form.credentials.proxyUrl ?? form.credentials.proxy_url ?? '').trim(),
)
const whatsappClientName = computed(() =>
  String(form.credentials.clientName ?? form.credentials.client_name ?? '').trim(),
)
