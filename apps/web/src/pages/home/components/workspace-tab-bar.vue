<template>
  <div class="flex min-h-11 shrink-0 items-stretch border-b border-border bg-sidebar select-none [-webkit-app-region:drag]">
    <div
      ref="tabsContainerRef"
      class="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto overflow-y-hidden whitespace-nowrap px-1.5 py-1 [-webkit-app-region:no-drag]"
    >
      <button
        v-for="tab in tabs"
        ...
        :key="tab.id"
        type="button"
        class="group inline-flex h-8 max-w-[190px] shrink-0 items-center gap-1.5 rounded-md border px-2.5 text-xs transition-colors [-webkit-app-region:no-drag]"
    ...
    <div class="flex shrink-0 items-center gap-0.5 border-l border-border px-1.5 py-1">
      <button
        type="button"
        class="inline-flex size-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-background/80 hover:text-foreground [-webkit-app-region:no-drag]"
